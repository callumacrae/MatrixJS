...
Matrix.isMatrix = function (a) {
	...
	// Validate array
	if (typeof a.array !== 'object' || !(a.array instanceof Array)) {
		return false;
	}

	for (var row = 0, col, l; row < a.array.length; row += 1) {
		if (typeof a.array[row] !== 'object' || !(a.array[row] instanceof Array)) {
			return false;
		}
		...
		for (col = 0; col < a.array[row].length; col += 1) {
			if (typeof a.array[row][col] !== 'number' && a.array[row][col] !== null) {
				return false;
			}
		}
	}
